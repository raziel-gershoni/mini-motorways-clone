...
- **"Unreachable because the sim freezes past X" can be TRUE and point the wrong way: the state was entered BEFORE the freeze, and the freeze is what makes it permanent.** M1f Task 8 drew a card modal as a second conditional draw phase and argued at the source that it could never coexist with the shutdown screen — *"`step` is a byte-identical no-op past the failure, so no week boundary can be crossed on a dead board"*. Every clause is true. The offer is raised at the boundary **before** the death, left unresolved, and then `H_OFFER_WEEK` can never catch up on a state that does not advance — so `offerPending` is frozen **true forever** and both phases ran, drawing two scrims a frame. It was caught by an existing allocation rig that boots an already-dead board, not by any test written for the modal, and it mattered: `pointer.ts` answers every tap on that screen with `RESTART_REQUESTED`, so the frame was asking a question the next tap did not answer. **When a freeze is your reachability argument, ask what the state was when it froze** — a no-op preserves a condition as reliably as it prevents one, and "cannot be entered from here" is not "cannot be in".
- **A reachability claim used to JUSTIFY a change needs measuring exactly as hard as one used to dismiss a bug — and "this makes the guard's fixture reachable" is the easiest one to attribute to the wrong constant.** The same task capped its modal's card height and justified it, in the commit message, the source, a catalogue entry and its report, with: *"uncapped, no board pixel is outside a tappable rect on any real viewport, so `REFUSED_OFFER_MODAL` would be a branch no fixture could enter."* **Measured afterwards by a reviewer: false.** Board sat outside every rect for three reasons the cap had nothing to do with — a 16 px side margin (10 px of actual board), a 12 px inter-card gap, and a 40 px title strip — and **the fixture in question probes the inter-card gap**. Raising the cap so the cards fill the band leaves the whole `game` package green and turns exactly three hand-computed `camera.test.ts` cases red. So the disclosed hazard was wrong in both directions: the property rested on different constants, and violating it fails **loudly** rather than becoming silently untestable. Two lessons, and the second is the one that generalises. **Name the constant the property actually rests on** — write the mutation, flip the constant, and read which tests move. And **"silently untestable" is a claim about your test suite, not an intuition**: check whether the change you fear would in fact go red first. The change itself was right for its other reason, which is what made the bad reason cheap to leave in.
