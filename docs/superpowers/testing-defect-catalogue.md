...
- **A full-screen modal can make its own refusal branch unreachable, so the LAYOUT is what gives a guard its fixture.** The same task's first layout filled the board band with two cards — a defensible design that left no board pixel outside a tappable rect on any real viewport, so `REFUSED_OFFER_MODAL`, the branch that exists to swallow a stray tap, was reachable only from a contrived camera. A guard with no reachable fixture is a guard with no detector, and nothing would have said so: the tests would simply have used whichever point worked. Capping the card height bought the fixture back, and the cap is now justified in the source by the reachability rather than only by the look. **Before writing a guard, ask where on the real screen the input it refuses can actually land.**
