...
**Throughput reality check.** Reproducible benchmarks (moving rects, 8,000 boxes) put vanilla Canvas2D at ~19 FPS in Chrome — but that cliff is at 8,000 sprites. Our workload is 200–400 moving sprites over a per-frame road layer (see the deleted baking bullet below), roughly an order of magnitude below it.
...
- ~~**Bake the road network to an offscreen canvas once per edit** and blit it as one `drawImage`.~~ **Deleted — M0 measured this as a pessimisation** [M0]. Draw road tiles per frame from the atlas instead. The reasoning, because it inverts the intuition this bullet was built on:
  - **The measured difference is not statistically real.** Baked 1.750 ms vs per-frame 1.600 ms at 400 sprites, but the ordering *flips with the statistic* — medians favour per-frame by 0.150 ms, means favour baked by 0.117 ms. With n=3 per arm the minimum achievable two-sided p is 0.10, so significance was unreachable before the data existed. The case against baking rests on pixel accounting, not on this measurement.
  - **The pixel accounting is not ambiguous.** Same playfield, same 400 sprites, same clear. Baked composites **3,178,980** device px in one `drawImage`; per-frame composites **995,328** across 432 — a **69% reduction**. The bake blits the whole 1218×2610 canvas while the grid occupies 1152×1920 of it and roads occupy 45% of *that*. **Baking composites 3.2× the pixels to draw the same roads.**
  - **We were optimising the cheap resource.** Back-solved from the intercept: ~10 Gpx/s and **~0.16 µs per `drawImage`**. Road layer: baked 0.318 ms (99.9% pixel cost), per-frame 0.168 ms (59% pixel, 41% call). WebKit batches consecutive `drawImage` calls from one atlas source into shared GPU draws, so 432 JS calls are not 432 GPU draws. **Pixel throughput binds; draw-call count is nearly free.** The deleted bullet's own arithmetic also never reconciled — the baked path still issues 400 sprite draws, so it is 401 not "~300", and 432 + 400 is 832, not "~1,500".
  - **Crossover is at 85% road density** as benchmarked, 59% if the bake were tightened to grid bounds, and **never** under perfect call batching, since the bake still blits the letterboxed area the tiles skip. **Measured density is 45%**, and typical Mini Motorways maps run below that.
  - **Three costs the benchmark never charged it:** +12.13 MiB of surface memory in a memory-constrained iOS WKWebView; a second full-canvas surface and an extra code path for a measured tie; and **invalidation — the core mechanic *is* drawing roads.** Every frame containing an edit costs re-render + offscreen clear + blit ≈ **0.81 ms, 4.8× the per-frame path**, and during a drag that is every frame.
