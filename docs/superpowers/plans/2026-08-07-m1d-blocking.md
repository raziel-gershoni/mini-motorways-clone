...
**1a. Consolidate `stepCell` into `roads.ts`.** Two copies — private at `cars.ts:155`, exported at `dispatch.ts:324`. `roads.ts` already owns `OPPOSITE`, `dirBetween` and `inBounds`, and is about to own `LANE_OF_DIR` too. M1c ruled to keep the duplication and paid for it — though **this plan stated the debt wrongly, and Task 1's implementer caught it.** The claim here was that `dispatch.ts`'s copy "had zero tests, with all four bounds surviving". False in the present tense: `dispatch.test.ts:1520-1580` has four direct `it()`s, one per bound. **The genuine gap was caller-side — 1 of 3 caller-observable bounds.** So 1a is a structural consolidation before a third caller arrives, not a coverage rescue, and the caller-side gap is the thing to close. The stale claim came from a carry-forward note written before M1c's own review closed part of it, copied forward without re-checking. Fold both copies in before adding a third caller.
...
**Coverage required:** both former `stepCell` call sites use the shared one; all four `stepCell` bounds are independently detected by **direct** calls; the three caller-observable bounds (`x < 0`, `x >= w`, `y >= h`) are additionally detected **through each caller**; `y < 0` carries a source comment recording it as a **verified equivalent mutant through either caller** — with the `x` guards retained, any `y ≤ −1` gives `y*w + x ≤ −1`, and both callers reduce every negative to one observable (`cars.ts` throws without interpolating `next`; `dispatch.ts:504-505` breaks). Re-verified exhaustively: 1,600 geometries × all in-range cells × Int32 extremes × 8 directions gave **0 differences in the sign**, with the largest differing mutant return `-2`. **The raw difference count is deliberately not stated.** An earlier draft cited 97,040; an independent exhaustive run gave 92,040 in-range and 118,640 including extremes, because the total depends entirely on which extreme cells are enumerated and no description fixes that. Only the sign result is load-bearing, and it reproduces robustly — a five-digit figure that cannot be re-derived from its own description reads as measurement without being any. **Do not manufacture a detector for it, and in particular do not tighten either caller's `next < 0` to `next === -1`** — that satisfies the bullet by strictly weakening two guards this milestone is about to hand a third caller. Also: `canPlaceRoad` is absent from a **dragging** profile, which is where that assertion can fail — reinstating the pre-fix allocation on all eight return paths measures 81.79 / 76.29 / 81.97 B/frame against a 4 B floor.

**The idle-profile arm is a control, not a detector, and this plan originally demanded it as though it were one.** With the defect fully reinstated, `idleBytes` is still 0 — no input means no `place` action, so `canPlaceRoad` never runs, and the assertion is unfalsifiable by construction. Keep it, because a non-zero idle figure would mean something else had gone wrong, but **label it as a control at the site** and put the vacuity guard on the drag arm where it can do work. Do not count it as coverage of this defect.
