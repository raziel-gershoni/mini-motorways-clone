...
- **Nothing allocates inside the frame loop**, and unlike the first draft this plan says what that means against `step`'s real object API — see Decision 9. Enforcement was originally construction, review, and one identity-based test (Task 6). **The claim that no allocation profiler exists was false** — `node:inspector`'s `HeapProfiler.startSampling` is a Node builtin, demonstrated during Task 6's review by measuring a reinstated allocation at ~112 B/frame. Task 6 now ships a real harness; this rule is mechanically enforced.
  > **The shipped harness is `packages/game/test/allocation.test.ts`** (~4.5 s, in the normal suite, zero new dependencies). It profiles 3,000 real frames of the full assembly and asserts that **no file under `packages/game/src` allocates beyond its budget** — every budget is 0 except `loop.ts`. Two things make it a guard rather than decoration, and both must survive any future edit: `includeObjectsCollectedByMinorGC` (without it only *survivors* are counted and every per-frame object is invisible), and a positive control that runs the **same predicate** over a profile containing one deliberate escaping object per frame. The assertion is **per file**, not per function, because attribution between adjacent *inlined* frames is genuinely unstable; per file it is not. Reinstating one escaping object inside `buildFrame` is charged to `frame.ts` at **53.5 B/frame** against a clean run where that file does not appear at all.
  >
  > **The harness immediately found a real violation nothing had ever seen.** `createLoop` held `lastTime`, `accumulator` and `alpha` as closure `let`s; a mutable double in a context slot boxes a fresh `HeapNumber` on **every assignment**, so the loop allocated **~65 B/frame — more than one object per frame**, against a rule this plan states as "zero, not small". Those three now live in a `Float64Array` with named slot indices, in `sim/state.ts`'s own idiom. Do not weaken the budgets to make a future change pass.
...
| `PHASE_OUTBOUND` → `PHASE_RETURNING` (the flip) | **`(330 − 2r) / threshold × \|dirVector\|`, SIGNED**, where `r` is the post-crossing carry in `[0, 330)` — forward at a small carry, backward at a large one. Bounded by 0.132 orthogonal / 0.1333 diagonal. **Measured on Task 2's seed: 0.07677 cells FORWARD at tick 406** (`r = 70`, diagonal) | Yes; the carry crosses the flip (trips.ts) and the reversed direction consumes it from the other side |
| `PHASE_RETURNING` → `PHASE_IDLE` (trip end) | **`(330 − r) / threshold × \|dirVector\|`, NOT zero.** Same bound. **Measured: 0.07677 at tick 435** (`r = 140`, diagonal). `completeTrip`'s `carCell = houseCell[carHome]` is a no-op on the *cell*; the resolved *position* still moves, because prev sits a fraction of a cell short of the house centre | Yes |
| **Largest ordinary driving tick — the supremum over the whole manifold** | `330/2500 = 0.132` orthogonal, `330·√2/3500 = 0.13334` diagonal, and the diagonal value is **attained** | — |
...
> **The first two rows were wrong in the first version of this plan (`≤ 0.076 backwards`, and `Zero`), and the errors were not merely descriptive — they made the *dangerous* threshold look safe.** An implementer reading the old table sees transitions bounded at 0.076 and would naturally site a distance guard at, say, **0.1** — comfortably above the largest discontinuity the old table admits. At θ = 0.1 the guard fires on **every ordinary driving tick** (0.132 / 0.13334), snapping every car to its curr position on every frame and destroying interpolation board-wide, while every tick-boundary position stays correct and every existing test passes. Both corrections were derived algebraically and confirmed by execution in Task 6, and again by an independently written resolver in its review.
>
> **And this is why "no distance guard" is right, stated precisely.** A distance guard *is* constructible — only the not-live-in-prev case can exceed 0.13334 — but it is **strictly subsumed**: `prevLive[i] === 0` detects that case with no false positives and no false negatives, while a threshold misses every sub-θ instance of it. No input reaches the distance guard alone, so its named must-fail mutation cannot fail, and the catalogue's "a check whose coverage is a strict subset of another check's is worth nothing" applies verbatim.
...
| `packages/game/src/frame.ts` | Builds the `RenderFrame` each frame: terrain fold, building unpack, dense car array, HUD scalars — **and `createFrameDriver`, the standard `LoopDriver`.** Added in Task 6 and recorded here so nobody re-litigates it: the ORDER of the two snapshots is the whole of Decision 2's correctness, and left in `main.ts` with a copy in each test rig, "snapshot prev after step" would have had no production mutation target for a whole task |
| `packages/game/src/loop.ts` | Accumulator, clock reference, pause. Its three real-valued slots live in a `Float64Array`, not in closure `let`s — see Global Constraints |
...
- **Allocation, stated as what it actually is:** across 1,000 frames with a live drag, the `TickInputs` object passed to `step` is identity-equal every time, and every `TickAction` in it is identity-equal to one previously seen. This kills "allocate a fresh action per event" and "allocate a fresh wrapper per tick" — the two allocations that exist here — and it does **not** prove global allocation-freedom. A `process.memoryUsage()` delta across 1,000 frames is dominated by GC timing, and the first draft asserted the property was unmeasurable in one section while requiring a test for it in another. **Both the first draft and this correction were still wrong about the tooling:** `node:inspector`'s `HeapProfiler.startSampling` is a Node builtin and attributes allocations by function name. Task 6's review proved it by measuring a reinstated allocation at ~112 B/frame against a clean baseline, so the identity test is now a supplement to a real profile rather than a substitute for one.
...
>
> **Wire Telegram's `MainButton` to `toggleEraseMode`.** Not a fourth canvas rect — that would move Task 5's fill-tiling arithmetic, which is asserted in device space and was the source of a ghosting seam. And **not a DOM button**, which was this note's first ruling and was withdrawn on the arithmetic: `fitCamera` leaves **49 CSS px** of free strip on a 390x844 viewport and **40** on the M0 reference device, both below a 44 px touch target, so "outside the canvas" is not a location on a phone. `MainButton` is native chrome — it reserves its own space so it costs no canvas geometry, it is thumb-reachable by construction, and it renders its own label and colour, which is what solves the real hazard: an erase mode you cannot see you are in is worse than no erase mode, because roads vanish under a drag the player thinks is drawing. Version-gate it like every other Telegram surface (§8.2's `atLeast`) and keep a plain DOM fallback for development outside Telegram and for clients below the gate.
>
